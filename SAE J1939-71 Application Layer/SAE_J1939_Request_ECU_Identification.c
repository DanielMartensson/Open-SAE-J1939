...
ENUM_J1939_STATUS_CODES SAE_J1939_Response_Request_ECU_Identification(J1939* j1939, uint8_t DA) {
	...
	if (length_of_each_field < 2) {
		...
		uint32_t ID = (0x18FDC5 << 8) | j1939->this_address;
		uint8_t data[8];
		...
		data[4] = j1939->this_ecu_identification.ecu_manufacturer[0];
		data[5] = j1939->this_ecu_identification.ecu_hardware_version[0];
		...
		uint8_t data[length_of_each_field*6];							/* Total 6 fields */
		for(uint8_t i = 0; i < length_of_each_field; i++) {
			...
			data[total_message_size++] = j1939->this_ecu_identification.ecu_manufacturer[i + length_of_each_field*4];
			data[total_message_size++] = j1939->this_ecu_identification.ecu_hardware_version[i + length_of_each_field*5];
		}
		...
		ENUM_J1939_STATUS_CODES status = J1939_Core_Send_TP_CM(DA, j1939->this_address, CONTROL_BYTE_TP_CM_BAM, total_message_size, number_of_packages, PGN_ECU_IDENTIFICATION);
		if(status != J1939_OK)
			return status;
...
void SAE_J1939_Read_Response_Request_ECU_Identification(J1939 *j1939, uint8_t SA, uint8_t data[]) {
	...
	uint8_t length_of_each_field = j1939->ecu_identification[SA].length_of_each_field;
	for(uint8_t i = 0; i < length_of_each_field; i++) {
		j1939->ecu_identification[SA].ecu_part_number[i] = data[i];
		j1939->ecu_identification[SA].ecu_serial_number[i] = data[i + length_of_each_field];
		j1939->ecu_identification[SA].ecu_location[i] = data[i + length_of_each_field*2];
		j1939->ecu_identification[SA].ecu_type[i] = data[i + length_of_each_field*3];
		j1939->ecu_identification[SA].ecu_manufacturer[i] = data[i + length_of_each_field*4];
		j1939->ecu_identification[SA].ecu_hardware_version[i] = data[i + length_of_each_field*5];
	}
}
