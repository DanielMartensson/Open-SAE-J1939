...
ENUM_J1939_STATUS_CODES SAE_J1939_Response_Request_DM2(J1939* j1939, uint8_t DA) {
	if(j1939->this_dm.errors_dm2_active < 2) {
		...
		ENUM_J1939_STATUS_CODES status = J1939_Core_Send_TP_CM(DA, j1939->this_address, CONTROL_BYTE_TP_CM_BAM, total_message_size, number_of_packages, PGN_DM2);
		if(status != J1939_OK)
			return status;
 ...
 * errors_dm2_active: How many errors we have. For example if errors_dm2_active = 2, then data[] contains 2*8 bytes
 * PGN: 0x00FECB (65227)
 */
