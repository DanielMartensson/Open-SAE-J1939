...
void Open_SAE_J1939_Listen_For_Messages(J1939* j1939) {
	...
	if(CAN_Read_Message(&ID, data)){
		...
		/* Read status information from other ECU */
			...
			ISO_11783_Read_Auxiliary_Estimated_Flow(j1939, SA, DA & 0xF, data);				/* DA & 0xF = Valve number. Total 16 valves from 0 to 15 */
		else if (id0 == 0x0C && id1 == 0xC6 && DA == j1939->this_ECU_address)
			ISO_11783_Read_General_Purpose_Valve_Estimated_Flow(j1939, SA, data);
		else if (id0 == 0x0C && id1 == 0xFF && DA >= 0x20 && DA <= 0x2F)
			ISO_11783_Read_Auxiliary_Valve_Measured_Position(j1939, SA, DA & 0xF, data); 	/* DA & 0xF = Valve number. Total 16 valves from 0 to 15 */
