...
void SAE_J1939_Read_Transport_Protocol_Connection_Management(J1939 *j1939, uint8_t SA, uint8_t data[]) {
	...
	if(j1939->tp_cm[SA].control_byte == CONTROL_BYTE_TP_CM_RTS)
		SAE_J1939_Send_Transport_Protocol_Connection_Management(j1939, SA, CONTROL_BYTE_TP_CM_CTS, j1939->tp_cm[SA].total_message_size, j1939->tp_cm[SA].number_of_packages, j1939->tp_cm[SA].PGN_of_the_packeted_message);
