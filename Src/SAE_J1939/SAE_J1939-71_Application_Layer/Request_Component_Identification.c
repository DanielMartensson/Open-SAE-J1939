...
ENUM_J1939_STATUS_CODES SAE_J1939_Response_Request_Component_Identification(J1939* j1939, uint8_t DA){
	...
	if (length_of_each_field < 2) {
		...
		for(uint8_t i = 0; i < length_of_each_field; i++) {
			data[i] = j1939->information_this_ECU.this_identifications.component_identification.component_product_date[i];
			data[length_of_each_field + i] = j1939->information_this_ECU.this_identifications.component_identification.component_model_name[i];
			data[length_of_each_field*2 + i] = j1939->information_this_ECU.this_identifications.component_identification.component_serial_number[i];
			data[length_of_each_field*3 + i] = j1939->information_this_ECU.this_identifications.component_identification.component_unit_name[i];
