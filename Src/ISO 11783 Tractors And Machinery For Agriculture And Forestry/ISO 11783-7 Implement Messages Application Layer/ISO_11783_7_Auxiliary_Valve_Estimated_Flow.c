 ...
 */
ENUM_J1939_STATUS_CODES ISO_11783_Send_Auxiliary_Valve_Estimated_Flow_To_All_ECU(J1939 *j1939, uint8_t valve_number) {
 ...
 * PGN: 0x00FE10 (65040) to 0x00FE1F (65055)
 */
void ISO_11783_Read_Auxiliary_Estimated_Flow(J1939 *j1939, uint8_t valve_number, uint8_t data[]) {
	j1939->auxiliary_valve_estimated_flow[valve_number].extend_estimated_flow_standard = data[0];
	j1939->auxiliary_valve_estimated_flow[valve_number].retract_estimated_flow_standard = data[1];
	j1939->auxiliary_valve_estimated_flow[valve_number].fail_safe_mode = data[2] >> 6;
	j1939->auxiliary_valve_estimated_flow[valve_number].valve_state = data[2] & 0b00001111;
	j1939->auxiliary_valve_estimated_flow[valve_number].limit = data[3] >> 5;
}
