...
void SAE_J1939_Read_Transport_Protocol_Data_Transfer(J1939 *j1939, uint8_t SA, uint8_t data[]) {
	...
	if(j1939->tp_cm[SA].control_byte == CONTROL_BYTE_TP_CM_RTS)
		SAE_J1939_Send_Acknowledgement(j1939, SA, CONTROL_BYTE_TP_CM_EndOfMsgACK, GROUP_FUNCTION_VALUE_NORMAL, j1939->this_ECU_address, PGN);
	...
	case PGN_COMMANDED_ADDRESS:
		...
		break;
	case PGN_DM1:
		SAE_J1939_Read_Response_Request_DM1(j1939, SA, complete_data, j1939->tp_dt[SA].sequence_number); /* Sequence number is the amount of errors */
		break;
	case PGN_DM2:
		SAE_J1939_Read_Response_Request_DM2(j1939, SA, complete_data, j1939->tp_dt[SA].sequence_number); /* Sequence number is the amount of errors */
		break;
	case PGN_DM16:
